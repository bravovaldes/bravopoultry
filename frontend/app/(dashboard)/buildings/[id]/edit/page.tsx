...
import { ArrowLeft, Building2, Save, AlertCircle, Loader2 } from 'lucide-react'
...
export default function EditBuildingPage() {
  ...
  // Populate form when building data is loaded
  useEffect(() => {
    if (building) {
      // Calculate length and width from surface if available
      let length_m = ''
      let width_m = ''
      if (building.surface_m2) {
        // Assume a ratio if we don't have exact dimensions
        const surface = Number(building.surface_m2)
        const estimatedLength = Math.sqrt(surface * 4) // Assume 4:1 ratio
        const estimatedWidth = surface / estimatedLength
        length_m = estimatedLength.toFixed(1)
        width_m = estimatedWidth.toFixed(1)
      }

      setFormData({
        name: building.name || '',
        code: building.code || '',
        building_type: building.building_type || 'layer',
        capacity: building.capacity?.toString() || '',
        length_m: length_m,
        width_m: width_m,
        ventilation_type: building.ventilation_type || 'natural',
        has_electricity: building.has_electricity ?? true,
        has_water: building.has_water ?? true,
  ...
  const updateBuilding = useMutation({
    mutationFn: async (data: any) => {
      ...
      const response = await api.patch(`/buildings/${buildingId}`, payload)
      return response.data
    },
    onSuccess: () => {
  ...
  if (success) {
    return <SuccessScreen title="Batiment mis a jour avec succes!" />
  }
    ...
    <div className="max-w-2xl mx-auto space-y-4 sm:space-y-5 lg:space-y-6">
      ...
      <div className="flex items-center gap-3 sm:gap-4">
        ...
        <div className="min-w-0 flex-1">
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 truncate">Modifier le batiment</h1>
